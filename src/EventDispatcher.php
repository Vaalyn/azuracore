...
class EventDispatcher extends \Symfony\Component\EventDispatcher\EventDispatcher
{
    ...
    public function addServiceSubscriber($class_name)
    {
        ...
        foreach ($class_name::getSubscribedEvents() as $eventName => $params) {
            if (\is_string($params)) {
                ...
                $this->addListener($eventName, $this->_getCallable($class_name, $params[0]), isset($params[1]) ? $params[1] : 0);
            } else {
                foreach ($params as $listener) {
                    $this->addListener($eventName, $this->_getCallable($class_name, $listener[0]), isset($listener[1]) ? $listener[1] : 0);
                }
            }
        }
    }
    ...
    public function removeServiceSubscriber($class_name)
    {
        ...
        foreach ($class_name::getSubscribedEvents() as $eventName => $params) {
            if (\is_array($params) && \is_array($params[0])) {
                ...
                $this->removeListener($eventName, $this->_getCallable($class_name, \is_string($params) ? $params : $params[0]));
            }
        }
    }
    ...
    {
        return new DeferredCallable($class_name.':'.$method, $this->di);
    }
